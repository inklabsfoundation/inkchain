...
func TestGenerateLocalMSP(t *testing.T) {
	...
	testMSPConfig, err := inkchainmsp.GetLocalMspConfig(mspDir, nil, testName)
	assert.NoError(t, err, "Error parsing local MSP config")
	testMSP, err := inkchainmsp.NewBccspMsp()
...
func TestGenerateVerifyingMSP(t *testing.T) {
	...
	testMSPConfig, err := inkchainmsp.GetVerifyingMspConfig(mspDir, testName)
