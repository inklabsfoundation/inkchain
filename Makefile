...
#   - peer - builds a native inkchain peer binary
...
#IMAGES = peer orderer ccenv javaenv buildenv tools zookeeper kafka couchdb
...
linter: buildenv
	...
	@$(DRUN) $(DOCKER_NS)/inkchain-buildenv:$(DOCKER_TAG) ./scripts/golinter.sh
...
build/docker/bin/%: $(PROJECT_FILES)
	...
	@$(DRUN) \
		...
		$(BASE_DOCKER_NS)/inkchain-baseimage:$(BASE_DOCKER_TAG) \
...
build/image/%/$(DUMMY): Makefile build/image/%/payload build/image/%/Dockerfile
	...
	$(DBUILD) -t $(DOCKER_NS)/inkchain-$(TARGET) $(@D)
	docker tag $(DOCKER_NS)/inkchain-$(TARGET) $(DOCKER_NS)/inkchain-$(TARGET):$(DOCKER_TAG)
	@touch $@
...
.PHONY: protos
protos: buildenv
	@$(DRUN) $(DOCKER_NS)/inkchain-buildenv:$(DOCKER_TAG) ./scripts/compile_protos.sh

%-docker-clean:
	$(eval TARGET = ${patsubst %-docker-clean,%,${@}})
	-docker images -q $(DOCKER_NS)/inkchain-$(TARGET) | xargs -I '{}' docker rmi -f '{}'
