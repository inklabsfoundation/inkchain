...
type queryHelper struct {
	...
	ledgerSetBuilder *ledgerutil.LedgerSetBuilder
	itrs             []*resultsItr
	err              error
	doneInvoked      bool
}
...
func (h *queryHelper) getState(ns string, key string) ([]byte, error) {
	...
	if h.ledgerSetBuilder != nil && h.ledgerSetBuilder.RwSetBuilder != nil {
		h.ledgerSetBuilder.RwSetBuilder.AddToReadSet(ns, key, ver)
	}
	return val, nil
}
...
func (h *queryHelper) getStateMultipleKeys(namespace string, keys []string) ([][]byte, error) {
	...
	for i, versionedValue := range versionedValues {
		val, ver := decomposeVersionedValue(versionedValue)
		if h.ledgerSetBuilder != nil && h.ledgerSetBuilder.RwSetBuilder != nil {
			h.ledgerSetBuilder.RwSetBuilder.AddToReadSet(namespace, keys[i], ver)
		}
		values[i] = val
	}
	return values, nil
...
func (h *queryHelper) getStateRangeScanIterator(namespace string, startKey string, endKey string) (commonledger.ResultsIterator, error) {
	h.checkDone()
	itr, err := newResultsItr(namespace, startKey, endKey, h.txmgr.db, h.ledgerSetBuilder.RwSetBuilder,
...
func (h *queryHelper) executeQuery(namespace, query string) (commonledger.ResultsIterator, error) {
	...
	return &queryResultsItr{DBItr: dbItr, RWSetBuilder: h.ledgerSetBuilder.RwSetBuilder}, nil
}
...
func (h *queryHelper) done() {
	...
	for _, itr := range h.itrs {
		if h.ledgerSetBuilder != nil && h.ledgerSetBuilder.RwSetBuilder != nil {
			...
			h.ledgerSetBuilder.RwSetBuilder.AddToRangeQuerySet(itr.ns, itr.rangeQueryInfo)
		}
	}
}
