...
func (historyDB *historyDB) Commit(block *common.Block) error {
	...
	for _, envBytes := range block.Data.Data {
		...
		if common.HeaderType(chdr.Type) == common.HeaderType_ENDORSER_TRANSACTION {
			...
			_, respPayload, err := putils.GetActionFromEnvelope(envBytes)
			if err != nil {
				return err
			}
			...
			if ledgerSet.TranSet != nil {
				compositeHistoryKey := historydb.ConstructCompositeHistoryKey(wallet.WALLET_NAMESPACE, ledgerSet.TranSet.From, blockNo, tranNo)
				dbBatch.Put(compositeHistoryKey, emptyValue)
				for _, trans := range ledgerSet.TranSet.KvTranSet.Trans {
					compositeHistoryKey = historydb.ConstructCompositeHistoryKey(wallet.WALLET_NAMESPACE, trans.To, blockNo, tranNo)
					dbBatch.Put(compositeHistoryKey, emptyValue)
				}
			}
