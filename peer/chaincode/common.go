...
func getSenderSpec(cmd *cobra.Command, cf *ChaincodeCmdFactory) (*pb.SenderSpec, error) {
	senderSpec := &pb.SenderSpec{}
	if len(priKey) != wallet.PriKeyStringLength {
		return nil, errors.New("invalid priKey")
	}
	address, err := wallet.GetAddressHexFromPrikey(priKey)
	if err != nil {
		return nil, err
	}
	input := &pb.ChaincodeInput{}
...
func chaincodeInvokeOrQuery(cmd *cobra.Command, args []string, invoke bool, cf *ChaincodeCmdFactory) (err error) {
	...
	var senderSpec *pb.SenderSpec
	var sig []byte
	if invoke && chaincodeName != "ascc" {
		senderSpec, err = getSenderSpec(cmd, cf)
		if err != nil {
			return err
		}
		signature, err := wallet.SignInvoke(spec, customIDGenAlg, senderSpec, priKey)
		if err != nil {
			return err
		}
		sig = signature
...
func ChaincodeInvokeOrQuery(
	...
	invocation := &pb.ChaincodeInvocationSpec{ChaincodeSpec: spec, SenderSpec: senderSpec, Sig: sig}
