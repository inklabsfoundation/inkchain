...
installChaincode () {
    peer chaincode install -n xc -v 1.0 -p ${XC_CC_PATH} -o orderer.example.com:7050 >&log.txt
    res=$?
    cat log.txt
...
instantiateChaincode () {
    ...
    peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C ${CHANNEL_NAME} -n xc -v 1.0 -c '{"Args":["init","INK","3c97f146e8de9807ef723538521fcecd5f64c79a"]}' -P "OR ('Org1MSP.member')" >&log.txt
    res=$?
    cat log.txt
