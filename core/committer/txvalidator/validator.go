...
import (
	...
	coreUtil "github.com/inklabsfoundation/inkchain/common/util"
...
func (v *vsccValidatorImpl) GetInfoForValidate(txid, chID, ccID string) (*sysccprovider.ChaincodeInstance, *sysccprovider.ChaincodeInstance, []byte, error) {
	...
	var err error
	/// ascc
	if ccID != "lscc" && ccID != "ascc" && ccID != "qscc" {
...
func (v *vsccValidatorImpl) VSCCValidateTx(payload *common.Payload, envBytes []byte, env *common.Envelope) (error, peer.TxValidationCode) {
	...
	_, respPayload, err := utils.GetActionFromEnvelope(envBytes)
	if err != nil {
	...
	if err = ledgerSet.FromProtoBytes(respPayload.Results); err != nil {
		return fmt.Errorf("ledgerSet.FromProtoBytes failed, error %s", err), peer.TxValidationCode_BAD_RWSET
	}
	...
	if !v.sccprovider.IsSysCC(ccID) {
		...
		//****************************** inkchain
		if ccID == wallet.WALLET_NAMESPACE {
