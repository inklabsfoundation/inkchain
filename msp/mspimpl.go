...
type bccspmsp struct {
	...
	cryptoConfig *m.InkchainCryptoConfig
}
...
func (msp *bccspmsp) Setup(conf1 *m.MSPConfig) error {
	...
	// given that it's an msp of type inkchain, extract the MSPConfig instance
	conf := &m.InkchainMSPConfig{}
	err := proto.Unmarshal(conf1.Config, conf)
	if err != nil {
		return fmt.Errorf("Failed unmarshalling inkchain msp config, err %s", err)
	}
...
func (msp *bccspmsp) setupCrypto(conf *m.InkchainMSPConfig) error {
	msp.cryptoConfig = conf.CryptoConfig
	if msp.cryptoConfig == nil {
		// Move to defaults
		msp.cryptoConfig = &m.InkchainCryptoConfig{
...
func (msp *bccspmsp) setupCAs(conf *m.InkchainMSPConfig) error {
...
func (msp *bccspmsp) setupAdmins(conf *m.InkchainMSPConfig) error {
	// make and fill the set of admin certs (if present)
	msp.admins = make([]Identity, len(conf.Admins))
	for i, admCert := range conf.Admins {
		id, _, err := msp.getIdentityFromConf(admCert)
...
func (msp *bccspmsp) setupCRLs(conf *m.InkchainMSPConfig) error {
...
func (msp *bccspmsp) finalizeSetupCAs(config *m.InkchainMSPConfig) error {
...
func (msp *bccspmsp) setupSigningIdentity(conf *m.InkchainMSPConfig) error {
...
func (msp *bccspmsp) setupOUs(conf *m.InkchainMSPConfig) error {
...
func (msp *bccspmsp) setupTLSCAs(conf *m.InkchainMSPConfig) error {
