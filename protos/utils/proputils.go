...
func GetActionFromEnvelope(envBytes []byte) (*peer.ChaincodeInvocationSpec, *peer.ChaincodeAction, error) {
...
func GetActionFromEnvelopePayload(payloadBytes []byte) (*peer.ChaincodeInvocationSpec, *peer.ChaincodeAction, error) {
	...
	ccPayload, respPayload, err := GetPayloads(tx.Actions[0])
	if ccPayload == nil {
		return nil, nil, fmt.Errorf("nil ChaincodeProposalPayload")
	}
	cpp, err := GetChaincodeProposalPayload(ccPayload.ChaincodeProposalPayload)
	if err != nil {
		return nil, nil, fmt.Errorf("GetChaincodeProposalPayload failed")
...
func GetChaincodeInvocationSpecFromSignedProposal(proposal *peer.Proposal) (*peer.ChaincodeInvocationSpec, error) {
	cpp, err := GetChaincodeProposalPayload(proposal.Payload)
	if err != nil {
		return nil, fmt.Errorf("GetChaincodeInvocationSpecFromProposal failed")
	}
	cis := &peer.ChaincodeInvocationSpec{}
	err = proto.Unmarshal(cpp.Input, cis)
	if err != nil {
