...
// afterGetAccount handles a GET_ACCOUNT request from the chaincode.
func (handler *Handler) afterGetAccount(e *fsm.Event, state string) {
	msg, ok := e.Args[0].(*pb.ChaincodeMessage)
	if !ok {
		e.Cancel(fmt.Errorf("Received unexpected message type"))
		return
	}
	chaincodeLogger.Debugf("[%s]Received %s, invoking get state from ledger", shorttxid(msg.Txid), pb.ChaincodeMessage_GET_ACCOUNT)
...
func (handler *Handler) afterGetFee(e *fsm.Event, state string) {
	msg, ok := e.Args[0].(*pb.ChaincodeMessage)
	if !ok {
		e.Cancel(fmt.Errorf("Received unexpected message type"))
		return
	}
	chaincodeLogger.Debugf("[%s]Received %s, invoking get state from ledger", shorttxid(msg.Txid), pb.ChaincodeMessage_GET_FEE)

	// Query ledger for state
	handler.handleGetFee(msg)
}
...
func (handler *Handler) handleGetAccount(msg *pb.ChaincodeMessage) {
	go func() {
		// Check if this is the unique state request from this chaincode txid
		uniqueReq := handler.createTXIDEntry(msg.Txid)
		if !uniqueReq {
		...
		var txContext *transactionContext
		txContext, serialSendMsg = handler.isValidTxSim(msg.Txid,
			"[%s]No ledger context for GetAccount. Sending %s", shorttxid(msg.Txid), pb.ChaincodeMessage_ERROR)

		defer func() {
			handler.deleteTXIDEntry(msg.Txid)
			if chaincodeLogger.IsEnabledFor(logging.DEBUG) {
				chaincodeLogger.Debugf("[%s]handleGetAccount serial send %s",
					shorttxid(serialSendMsg.Txid), serialSendMsg.Type)
			}
			handler.serialSendAsync(serialSendMsg, nil)
		}()
		...
		if chaincodeLogger.IsEnabledFor(logging.DEBUG) {
			chaincodeLogger.Debugf("[%s] getting account for key %s, channel %s",
				shorttxid(msg.Txid), address, txContext.chainID)
		}

		var res []byte
		var err error
		res, err = txContext.txsimulator.GetState(wallet.WALLET_NAMESPACE, address)

		if err != nil {
			// Send error msg back to chaincode. GetState will not trigger event
			payload := []byte(err.Error())
			chaincodeLogger.Errorf("[%s]Failed to get balance(%s). Sending %s",
				shorttxid(msg.Txid), err, pb.ChaincodeMessage_ERROR)
			serialSendMsg = &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_ERROR, Payload: payload, Txid: msg.Txid}
		} else if res == nil {
			//The state object being requested does not exist
			chaincodeLogger.Debugf("[%s]account %s doesn't exist. Sending %s with an empty payload",
				shorttxid(msg.Txid), address, pb.ChaincodeMessage_RESPONSE)
			serialSendMsg = &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_RESPONSE, Payload: res, Txid: msg.Txid}
		} else {
			// Send response msg back to chaincode. GetState will not trigger event
			if chaincodeLogger.IsEnabledFor(logging.DEBUG) {
...
func (handler *Handler) handleGetFee(msg *pb.ChaincodeMessage) {
	go func() {
		// Check if this is the unique state request from this chaincode txid
		uniqueReq := handler.createTXIDEntry(msg.Txid)
		if !uniqueReq {
		...
		defer func() {
			handler.deleteTXIDEntry(msg.Txid)
			if chaincodeLogger.IsEnabledFor(logging.DEBUG) {
				chaincodeLogger.Debugf("[%s]handleGetFee serial send %s",
					shorttxid(serialSendMsg.Txid), serialSendMsg.Type)
			}
			handler.serialSendAsync(serialSendMsg, nil)
		}()
		...
		if err != nil {
			// Send error msg back to chaincode. GetState will not trigger event
			payload := []byte(err.Error())
			chaincodeLogger.Errorf("[%s]Failed to get fee(%s). Sending %s",
				shorttxid(msg.Txid), err, pb.ChaincodeMessage_ERROR)
			serialSendMsg = &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_ERROR, Payload: payload, Txid: msg.Txid}
		} else {
			// Send response msg back to chaincode. GetState will not trigger event
...
func (handler *Handler) handleVerify(msg *pb.ChaincodeMessage) {
	go func() {
		...
		if err != nil {
			// Send error msg back to chaincode. GetState will not trigger event
			payload := []byte(err.Error())
			chaincodeLogger.Errorf("[%s]Failed to get signature check results(%s). Sending %s",
				shorttxid(msg.Txid), err, pb.ChaincodeMessage_ERROR)
			serialSendMsg = &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_ERROR, Payload: payload, Txid: msg.Txid}
		} else {
			result := true
...
func (handler *Handler) enterBusyState(e *fsm.Event, state string) {
	go func() {
		...
		if msg.Type.String() == pb.ChaincodeMessage_PUT_STATE.String() {
		...
		} else if msg.Type.String() == pb.ChaincodeMessage_ISSUE_TOKEN.String() {
			issueTokenInfo := &pb.IssueTokenInfo{}
			unmarshalErr := proto.Unmarshal(msg.Payload, issueTokenInfo)
			if unmarshalErr != nil {
				errHandler([]byte(unmarshalErr.Error()), "[%s]Unable to decipher payload. Sending %s", shorttxid(msg.Txid), pb.ChaincodeMessage_ERROR)
				return
			}
			chaincodeID := handler.getCCRootName()
			addressStr := string(issueTokenInfo.Address[:])
			// verify ascc
			if chaincodeID != "ascc" {
				errHandler([]byte("issue token error"), "[%s]Issue token can only be used by ascc. Sending %s", shorttxid(msg.Txid), pb.ChaincodeMessage_ERROR)
				return
			}
			account := &wallet.Account{}
			var res []byte
			var err error
			res, err = txContext.txsimulator.GetState(wallet.WALLET_NAMESPACE, addressStr)

			if err == nil {
				err = json.Unmarshal(res, account)
				if err == nil && account.Balance != nil {
					balance, ok := account.Balance[string(issueTokenInfo.BalanceType[:])]
					if ok && balance.Cmp(big.NewInt(0)) > 0 {
						errHandler([]byte(unmarshalErr.Error()), "[%s]Token exists. Sending %s", shorttxid(msg.Txid), pb.ChaincodeMessage_ERROR)
						return
					}
				}
			}
			...
			var kvTrans []*kvtranset.KVTrans
			for _, tran := range transferInfo.TranSet {
				kvTran := kvtranset.KVTrans{}
				kvTran.To = string(tran.To[:])
				kvTran.BalanceType = string(tran.BalanceType[:])
				kvTran.Amount = tran.Amount
				kvTrans = append(kvTrans, &kvTran)
			}
			tranSet := &kvtranset.KVTranSet{Trans: kvTrans}
...
func (handler *Handler) HandleMessage(msg *pb.ChaincodeMessage) error {
	chaincodeLogger.Debugf("[%s]Inkchain side Handling ChaincodeMessage of type: %s in state %s", shorttxid(msg.Txid), msg.Type, handler.FSM.Current())
