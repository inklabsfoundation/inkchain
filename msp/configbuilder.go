...
func getMspConfig(dir string, ID string, sigid *msp.SigningIdentityInfo) (*msp.MSPConfig, error) {
	...
	// otherwise skip it
	var ouis []*msp.InkchainOUIdentifier
	...
	// Set InkchainCryptoConfig
	cryptoConfig := &msp.InkchainCryptoConfig{
		SignatureHashFamily:            bccsp.SHA2,
		IdentityIdentifierHashFunction: bccsp.SHA256,
	}

	// Compose InkchainMSPConfig
	fmspconf := &msp.InkchainMSPConfig{
		Admins:            admincert,
		RootCerts:         cacerts,
		IntermediateCerts: intermediatecerts,
	...
	mspconf := &msp.MSPConfig{Config: fmpsjs, Type: int32(INKCHAIN)}
