...
func initPeer(chainID string) (*testEnvironment, error) {
	...
	tempDir := newTempDir()
	viper.Set("peer.fileSystemPath", filepath.Join(tempDir, "inkchain", "production"))
...
func TestDeployAndInvoke(t *testing.T) {
	...
	if err == nil {
		t.Fail()
		t.Logf("expecting inkchain to report error from chaincode failure")
		chaincode.GetChain().Stop(ctxt, cccid, &pb.ChaincodeDeploymentSpec{ChaincodeSpec: &pb.ChaincodeSpec{ChaincodeId: chaincodeID}})
		return
...
func newTempDir() string {
	tempDir, err := ioutil.TempDir("", "inkchain-")
	if err != nil {
		panic(err)
	}
	return tempDir
