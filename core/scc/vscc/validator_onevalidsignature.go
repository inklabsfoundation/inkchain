...
func (vscc *ValidatorOneValidSignature) ValidateLSCCInvocation(stub shim.ChaincodeStubInterface, chid string, env *common.Envelope, cap *pb.ChaincodeActionPayload, payl *common.Payload) error {
	...
	case lscc.UPGRADE, lscc.DEPLOY:
		...
		if err = ledgerSet.FromProtoBytes(respPayload.Results); err != nil {
			return fmt.Errorf("ledgerSet.FromProtoBytes error %s", err)
		}
		...
		for _, ns := range txRWSet.NsRwSets {
			if ns.NameSpace != "lscc" && ns.NameSpace != wallet.WALLET_NAMESPACE && ns.NameSpace != cdRWSet.Name && len(ns.KvRwSet.Writes) > 0 {
