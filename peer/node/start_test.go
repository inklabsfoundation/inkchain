...
func TestStartCmd(t *testing.T) {
	...
	viper.Set("peer.fileSystemPath", "/tmp/inkchain/test")
...
loop:
	for {
		...
		default:
			_, err := os.Stat("/tmp/inkchain/test/peer.pid")
			if err != nil {
	...
	pidFile, err := ioutil.ReadFile("/tmp/inkchain/test/peer.pid")
	if err != nil {
		t.Fail()
	...
	os.RemoveAll("/tmp/inkchain/test")
}
...
func TestWritePid(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			fileName: "/tmp/inkchain/test/peer.pid",
			pid:      os.Getpid(),
			expected: true,
		},
		{
